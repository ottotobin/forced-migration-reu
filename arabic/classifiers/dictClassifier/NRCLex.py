...
#Global variables that should be changed if used by different groups
LEXICON_TSV_PATH = "lexicon/Arabic-NRC-EmoLex.tsv"
EMOTIONS = ['anger', 'fear', 'sadness', 'disgust', 'joy', 'surprise']
LEXICON_JSON_PATH = 'lexicon/Arabic-NRC-EmoLex.json'
LANGUAGE = "Arabic"
...
class NRCLex:
    ...
    #Include a function to transform the lexicon to a
    #json which can be used in for Arabic. This is a necessary
    #change because the original code only allows for English.
    def lex_to_json(tsv_file):
        ...
        for index, row in df.iterrows():
            ...
            dict[row[LANGUAGE+' Word']] = emotion_list      

        with open(LEXICON_JSON_PATH, 'w+') as outfile:
            json.dump(dict, outfile, indent=4, ensure_ascii=False)

    lex_to_json(LEXICON_TSV_PATH)
    with open(LEXICON_JSON_PATH, "r") as json_file:
