...
def time_series(file):
    org_type = file.strip('output/').split("_")[0]
...
def corr_matrix(trends_df, file):
    org_type = file.strip('output/').split("_")[0]
...
def main():
    ...
    parser.add_argument("-label", default="output/label_outfile.csv")
    parser.add_argument("-keyword", default="output/gtrKey_outfile.csv")
    parser.add_argument("-loc", default="output/gtrLoc_outfile.csv")
    args = parser.parse_args()

    for file in args.__dict__.values():
