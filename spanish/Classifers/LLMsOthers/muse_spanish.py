...
def main(args):
   # Load data and preprocess tweets
    data = pd.read_csv(args.filename, sep='\t', on_bad_lines='skip')
    ...
    # Initialize the variations dictionary with the default hyperparameters
    variations = [
        {'learning_rate': args.learningrate, 'epochs': args.epochs, 'momentum': args.momentum, 'batch_size': args.batchsize}
    ]
    ...
    for vari in variations:
        ...
        accuracy_dict, f1_dict, loss_dict, true_pos_dict, false_pos_dict, true_neg_dict, false_neg_dict = train_and_evaluate(
            X_train_tensor, X_test_tensor, Y_train_tensor, Y_test_tensor, learning_rate, epochs, momentum, batch_size
        )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Emotion Detection - MUSE LLM")
    ...
    parser.add_argument("--learningrate", type=float, default=0.00001, help="Set learning rate. Default is 0.00001.")
    parser.add_argument("--epochs", type=int, default=10, help="Set number of epochs. Default is 10.")
    parser.add_argument("--momentum", type=float, default=0.9, help="Set momentum for the optimizer. Default is 0.9.")
