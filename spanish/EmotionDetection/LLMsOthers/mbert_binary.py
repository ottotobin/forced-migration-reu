...
sys.stdout = open("mbert_binary.txt", "w")

#read in data
data = pd.read_csv('spanish_emotion.tsv', sep='\t', on_bad_lines='skip')
data_filter = data[~data['emotion'].isin(['surprise', 'disgust'])]
tweets = data_filter['tweet']
tweets = [emoji.demojize(str(tweet), language='es') for tweet in tweets]

emotions = ['joy', 'sadness', 'fear', 'others', 'anger']
for emotion in emotions:

    # splitting data for training and testing sets
    train_texts, test_texts, train_labels, test_labels = train_test_split(tweets, data_filter['emotion'], stratify=data_filter['emotion'], test_size=0.2)
    ...
    train_encoding = tokenizer(train_texts, return_tensors='pt', padding=True, truncation=True, max_length=512)
    ...
    test_encoding = tokenizer(test_texts, return_tensors='pt', padding=True, truncation=True, max_length=512)
    ...
    optimizer = torch.optim.NAdam(optimizer_grouped_parameters, lr=1e-5)
    ...
    try:
        ...
        print("Confusion Matrix: \n", eval_cm)
        print()
        torch.save(model.state_dict(), "model_bert_others.pt")
